...
Node convert<spartan3::ConfigurationBus>::encode(
    const spartan3::ConfigurationBus &rhs) {
  Node node;
  node.SetTag("xilinx/spartan3/configuration_bus");
  node["configuration_columns"] = rhs.configuration_columns_;
  return node;
}

bool convert<spartan3::ConfigurationBus>::decode(
    const Node &node, spartan3::ConfigurationBus &lhs) {
  if (!node.Tag().empty() &&
      node.Tag() != "xilinx/spartan3/configuration_bus") {
    return false;
  }

  lhs.configuration_columns_ =
      node["configuration_columns"]
          .as<std::map<unsigned int, spartan3::ConfigurationColumn>>();
  return true;
}
